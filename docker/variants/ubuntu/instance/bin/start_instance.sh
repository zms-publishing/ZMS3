...
# Stream startup logs to container output, but only print the first 1000 lines.
nohup "$VENV_DIR/bin/runzope" -C "$INSTANCE_DIR/etc/zope_$HTTP_PORT.conf" 2>&1 | awk 'NR<=1000 { print; fflush() }' &
